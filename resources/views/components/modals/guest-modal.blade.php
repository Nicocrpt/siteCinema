    ...
    <div @click.away="guestModal = false" class="overflow-hidden w-[95%] max-w-[700px] h-1/2 bg-gray-50 dark:bg-zinc-900 rounded-md shadow-lg flex flex-col justify-center gap-2 items-center z-50">
        <div class="w-full max-w-[500px] flex flex-col gap-2 justify-center items-center overflow-hidden">
            ...
            <div class="flex transition-all ease-in-out duration-300 w-full rounded-lg" :style="translate ? `transform: translateX(-${translate}%)` : ``">
                <div class="w-full flex-shrink-0 flex flex-col gap-2 z-50 px-4" x-transition:enter="transition-all ease-in-out duration-500" x-transition:enter-start="opacity-100" x-transition:enter-end="opacity-0" x-transition:leave="transition-all ease-in-out duration-500" x-transition:leave-start="opacity-0" x-transition:leave-end="opacity-100">
                    <a href="{{ route('login') }}" class=" w-full h-12 flex-shrink-0 bg-cyan-700 hover:bg-cyan-600 text-white py-2 px-4 border-l border-l-cyan-900 rounded-md transition-all ease-in-out duration-300 flex justify-center items-center"><span>Me connecter</span></a>
                    <button @click="translate = 100" class=" w-full h-12 flex-shrink-0 bg-cyan-700 hover:bg-cyan-600 text-white py-2 px-4 border-l border-l-cyan-900 rounded-md transition-all ease-in-out duration-300">Réserver en tant qu'invité</button>
                </div>
