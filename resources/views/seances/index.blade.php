...
@section('content')
    <div x-data="seancesPage()" class="h-full bg-zinc-100 dark:bg-zinc-900 relative w-full">
        <div class="fixed top-0 w-full">
            <section class="w-full !h-[5.5rem] bg-zinc-100 dark:bg-zinc-900 mt-[56px] flex items-center justify-center relative">

                <button class="hover:bg-zinc-300 dark:hover:bg-zinc-600 p-2 rounded" @click="prev()" :class="today >= firstDate ? 'pointer-events-none' : ''">
                ...
                <div class="!overflow-hidden max-w-[18rem] 4-items:max-w-[24rem] 5-items:max-w-[30rem] 6-items:max-w-[36rem] md:max-w-[42rem]  mx-1 lg:mx-4 pt-2 h-full relative rounded" id="slider">
                    <div class=" h-full flex ease-in-out duration-700 rounded-full" :style="`transform: translateX(${translation}rem);`" id="dateContainer">
        
                    </div>
                </div>
                <button class="hover:bg-zinc-300 dark:hover:bg-zinc-800 p-2 rounded" @click="next()">
