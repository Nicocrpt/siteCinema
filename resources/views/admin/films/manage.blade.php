...
@if (session('success'))
    <div id="success" class="absolute top-[3.25rem] w-full flex justify-center items-center rounded transition-all ease-in-out duration-300 -translate-y-[120%]">
        <p class="bg-green-600 border border-green-700 text-white p-1 rounded">{{ session('success') }}</p>
    </div>
@endif
        ...
        <div class="w-full flex max-h-[calc(100vh-16rem)] flex-wrap gap-2 mt-4 overflow-y-auto bg-zinc-100 border border-zinc-400 dark:border-zinc-500 dark:bg-zinc-600 rounded p-2" id="filmsList">
            @foreach ($films as $film)
                ...
                <div class="relative group w-48 overflow-hidden rounded shadow-sm" x-data="{miniMenu: false, estFavori: {{ $film->est_favori ? '1' : '0' }}}" x-init="document.querySelectorAll('.menu').forEach(el => el.classList.remove('hidden'))"/>
                    <svg width="22" class="favoriteLabel absolute hover:backdrop-blur-[1.5px] z-30 top-2 left-2 pointer-events-none" :class="estFavori ? 'fill-yellow-300 dark:fill-yellow-400' : 'fill-zinc-300 dark:fill-zinc-600 opacity-60'" viewBox="0 0 64 64"  xmlns="http://www.w3.org/2000/svg">
